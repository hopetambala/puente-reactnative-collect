...
// A partially-failed sync leaves the WHOLE batch queued on the device, so the
// next Retry re-sends records that already saved. Cloud Code dedupes on
// `objectIdOffline`, which it derives from a local id — `PatientID-` for
// residents, `Household-` for households, `SupID-` for supplementary forms.
//
// It has always had that SupID- branch. The client has never stamped one, so
// supplementary forms reach the server with no idempotency key at all and a
// retry after a partial failure creates DUPLICATE health records — silently,
// and more of them on every further retry.
describe("postSupplementaryForm offline — idempotency key for retries", () => {
  ...
  it("stamps a SupID- local id so a re-sent record dedupes", async () => {
    await postSupplementaryForm({
      ...
      parseParentClassID: "parent1",
      localObject: { fieldA: 1 },
    });

    expect(queuedBy("offlineSupForms").localObject.objectId).toMatch(/^SupID-/);
  });
  ...
  it("gives the asset supplementary queue the same key", async () => {
    ...
    expect(queuedBy("offlineAssetSupForms").localObject.objectId).toMatch(
      /^SupID-/
    );
  });

  it("keeps the id stable so a record queued once is not re-keyed", async () => {
    const already = {
      parseClass: "Vitals",
      parseParentClassID: "parent1",
      localObject: { objectId: "SupID-original" },
    };

    await postSupplementaryForm(already);

    expect(queuedBy("offlineSupForms").localObject.objectId).toBe(
      "SupID-original"
    );
  });

  // The online path posts straight to Parse, where an unknown objectId is
  // rejected outright. The key belongs only to records that go to the queue.
  it("does not stamp one on a record that posts online", async () => {
    checkOnlineStatus.mockResolvedValue(true);
    postObjectsToClassWithRelation.mockResolvedValue({ objectId: "real" });
    fulfillWithTimeLimit.mockResolvedValue({ value: { objectId: "real" } });
    ...
    await postSupplementaryForm(params);
